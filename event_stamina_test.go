...
func TestStaminaEventMakesNonfiniteEvidenceAbsent(t *testing.T) {
	invalid := float32(math.NaN())
	p := &Parser{clock: &Clock{}}
	p.appendStaminaConsumedEvent(64, &protocol.CCitadelUserMsg_StaminaConsumed{
		StaminaBefore: &invalid, StaminaAfter: &invalid, StaminaMax: &invalid, Gametime: &invalid,
	})
	...
	if _, err := json.Marshal(event); err != nil {
		t.Fatalf("stamina event cannot be emitted as JSON: %v", err)
	}
	stamina := event.StaminaConsumed
	if stamina.HasStaminaBefore || stamina.HasStaminaAfter || stamina.HasStaminaMax || stamina.HasMessageGameTime {
		t.Fatal("nonfinite stamina fields retained presence")
	}
}

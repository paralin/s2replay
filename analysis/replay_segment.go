...
func rowComplete(row ReplaySegmentRow) bool {
	fields := []ReplayScalar{row.PositionX, row.PositionY, row.PositionZ, row.FacingX, row.FacingY, row.FacingZ, row.VelocityX, row.VelocityY, row.VelocityZ}
	for _, field := range fields {
		if field.Quality != ReplayFieldExact {
			return false
		}
	}
	return true

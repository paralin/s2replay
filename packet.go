...
// appendAbilityNotifyEvent preserves message presence and observed entity indexes.
...
// appendImportantAbilityUsedEvent attributes full handles to active incarnations.
func (p *Parser) appendImportantAbilityUsedEvent(tick uint32, msg *protocol.CCitadelUserMessage_ImportantAbilityUsed) {
	player := int32(-1)
	slot := int32(-1)
	if msg.Player != nil {
		if playerEntity := p.FindEntityByHandle(uint64(msg.GetPlayer())); playerEntity != nil && playerEntity.active {

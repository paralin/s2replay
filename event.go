...
func sanitizeEvent(ev *Event) {
	...
	if stamina := ev.StaminaConsumed; stamina != nil {
		for _, field := range []struct {
			value   *float32
			present *bool
		}{
			{&stamina.StaminaBefore, &stamina.HasStaminaBefore},
			{&stamina.StaminaAfter, &stamina.HasStaminaAfter},
			{&stamina.StaminaMax, &stamina.HasStaminaMax},
			{&stamina.MessageGameTime, &stamina.HasMessageGameTime},
		} {
			if !isFiniteFloat32(*field.value) {
				*field.value = 0
				*field.present = false
			}
		}

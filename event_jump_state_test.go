...
func TestAppendJumpStateEventSeparatesEpochsAndChecksOwnerSerial(t *testing.T) {
	...
	owner.active = false
	values["m_hOwnerEntity"] = uint32(owner.serial<<14) | uint32(owner.index)
	p.appendJumpStateEvent(23, jumpTestEntity(22, 1, values))
	if p.pendingEvents[2].PlayerSlot != -1 {
		t.Fatalf("inactive owner attributed to slot %d", p.pendingEvents[2].PlayerSlot)
	}
}

...
func TestAppendImportantAbilityUsedEventPreservesPresenceAndValidatesHandle(t *testing.T) {
	...
	p.pendingEvents = nil
	p.appendImportantAbilityUsedEvent(35, &protocol.CCitadelUserMessage_ImportantAbilityUsed{Player: &player})
	if p.pendingEvents[0].PlayerSlot != -1 {
		t.Fatalf("inactive player attributed to slot %d", p.pendingEvents[0].PlayerSlot)
	}
}
